...
public class SubscriptionController {
    ...
    public ResponseEntity createSubscription(@RequestParam("subscriptionPlan") String subscriptionPlan, @RequestParam("email") String username, @RequestParam("password") String password) throws Exception {
        ...
        if (account == null) {
            ...
            accountRepo.add(new Account(username, password, AccountRole.DISABLED, null, null));
        }
    ...
    public ResponseEntity cancel() throws StripeException {
        ...
        Subscription.retrieve(Objects.requireNonNull(Util.SUBSCRIPTION_ID())).setCancelAtPeriodEnd(true);
        subject.logout();
        return new ResponseEntity(HttpStatus.OK);
    }
    ...
    private ResponseEntity updateExistingAccountIfNeeded(Session hooksession) throws SQLException, StripeException {
        ...
        if (Util.LOGGED_IN_ACCOUNT_COMMUNITYID() != null)
