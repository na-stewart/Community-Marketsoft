...
public abstract class Repo<T> {
    ...
    public T get(int id) throws SQLException {
        ...
        try (Connection connection = DbUtil.DATA_SOURCE.getConnection();
            PreparedStatement statement = connection.prepareStatement(selectQuery + " WHERE id = ? AND communityId = ?")) {
                ...
                statement.setString(2, Util.LOGGED_IN_ACCOUNT_COMMUNITYID());
    ...
    public List<T> getAll() throws SQLException {
        ...
        try (Connection connection = DbUtil.DATA_SOURCE.getConnection();
            ...
            statement.setString(1, Util.LOGGED_IN_ACCOUNT_COMMUNITYID());
            try (ResultSet resultSet = statement.executeQuery()){
    ...
    public List<T> getAll(String whereClause, String param) throws SQLException {
        return getAll(whereClause, new String[]{param});
    }


    public List<T> getAll(String whereClause, String[] params) throws SQLException {
        int i = 0;
        List<T> list = new ArrayList<>();
        try (Connection connection = DbUtil.DATA_SOURCE.getConnection();
             PreparedStatement statement = connection.prepareStatement(selectQuery + " " + whereClause + " AND communityId = ?")) {
           while(i < params.length) {
               statement.setString(i + 1, params[i]);
               i++;
           }
           statement.setString(i + 1, Util.LOGGED_IN_ACCOUNT_COMMUNITYID());
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next())
    ...
    public void delete(int id, boolean log) throws SQLException {
        try (Connection connection = DbUtil.DATA_SOURCE.getConnection();
            ...
            if (log)
                accountActivityRepo().add(new AccountActivity(ActivityType.DELETE, (Model) get(id)));
        }
    }
