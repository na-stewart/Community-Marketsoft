...
public class EmployeeController implements InitGUI {
    ...
    @FXML
    private VBox panelNameVbox;
    @FXML
    private VBox panelImageVbox;
    @FXML
    ...
    public void initialize(URL location, ResourceBundle resources) {
        setCellValueFactory();
        initSelectablePanels();
        loadPanelsChoiceBox();
        helpWebView.getEngine().load("https://www.communitymarketsoft.com");
        loadDataManagerEventHandlers();
        disableNotAllowedPanels();
        setTableSelectionMethods();
    }


    private void initSelectablePanels(){
        panels = new EmployeePanel[]{new EmployeePanel(customersAnchorPane, "Customers"),
                new EmployeePanel(accountsAnchorPane, "Accounts"), new EmployeePanel(logsTabPane, "Logs"),
                new EmployeePanel(itemsAnchorPane, "Items"), new EmployeePanel(helpAnchorPane, "Help")};
    }
    ...
    private void disableNotAllowedPanels(){
        if (!AuthUtil.hasRole(AccountRole.ADMIN)){
            logsTabPane.getTabs().get(0).setDisable(true);
            logsTabPane.getSelectionModel().select(1);
            removePanel(2);
        }
    }

    private void removePanel(int index){
        panelNameVbox.getChildren().remove(index);
        panelImageVbox.getChildren().remove(index);
    }
    ...
    private void logout() {
        AuthUtil.LOGOUT();
        Stage stage = (Stage) helpAnchorPane.getScene().getWindow();
        stage.close();
