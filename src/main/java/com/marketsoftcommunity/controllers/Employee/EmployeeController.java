...
import main.java.com.marketsoftcommunity.consumers.AccountDTO;
...
import main.java.com.marketsoftcommunity.model.logging.Announcement;
...
//Change choice box's to item type category.
public class EmployeeController implements InitGUI {
    ...
    private final GeneralDTO<Announcement> announcementLogger = new GeneralDTO<>("announcement", Announcement.class);
    ...
    @FXML
    private ChoiceBox<ItemCategory> itemsBoughtFrequencyChoiceBox;
    @FXML
    ...
    private void openPanel(MouseEvent mouseEvent) {
        for (EmployeePanel guiPanel : panels) {
            if (guiPanel.getName().equals(((Text) mouseEvent.getSource()).getText())) {
                if (guiPanel.getRequiresPermissions())
    ...
    private void populateAnnouncementsTextArea() throws IOException {
    ...
    private void populateAccountActivityFrequency() throws IOException{
        int frequency = 0;
    ...
    private List<String> listOfItemNames() throws IOException {
        List<String> list = new ArrayList<>();
        for (PurchasesActivity purchasesActivity : purchasesActivityLogger.getAll("WHERE itemtype = ?",
                itemsBoughtFrequencyChoiceBox.getValue().getName())) {
    ...
    */
    ...
    private void createItemsPanelEventHandler() {
        EventHandler<Event> itemEventHandler = new EmployeePanelHandler<Item>(itemsTableView, itemDTO, "items") {
    ...
    private void populateItemTypeChoiceBox() {
        try {
            for (ItemCategory itemcategory: itemDTO.getAllCategories()) {
                ...
                itemsBoughtFrequencyChoiceBox.getItems().add(itemcategory);
            }
        } catch (IOException e) {
            e.printStackTrace();
            LoggingUtil.logExceptionToFile(e);
    ...
    private void createCustomerPanelEventHandler() {
        EventHandler<Event> customerEventHandler = new EmployeePanelHandler<Customer>(customerTableView, customerDTO, "customers") {
    ...
    private void createAccountPanelEventHandler() {
        EventHandler<Event> accountEventHandler = new EmployeePanelHandler<Account>(accountTableView, accountDTO, "accounts") {
    ...
    */
    ...
    private void addPermission() throws Exception {
          Account account = accountTableView.getSelectionModel().getSelectedItem();
          AccountPermission accountPermission = new AccountPermission(account.getName(), permissionsField.getText(), account.getId());
          accountDTO.addPermissions(accountPermission);
    ...
    private void deletePermission() throws Exception {
        for (AccountPermission accountPermission : accountPermissionTableView.getSelectionModel().getSelectedItems()) {
            accountDTO.deletePermission(accountPermission.getId());
            accountActivityLogger.add(new AccountActivity(ActivityType.DELETE, accountPermission));
        }
      tryToDisplayPermissions();
    }

    private void tryToDisplayPermissions(){
        try {
            accountPermissionTableView.getItems().setAll(accountDTO.getPermissions(usernameField.getText()));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
